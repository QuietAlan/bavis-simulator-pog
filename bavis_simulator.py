...
# then if the user replies with "bavis", one "bavis" gets added to the bavis_counter variable.
    if bavis == 'bavis':
        bavis_counter += 1*multiplier + (0.25*prestige) + (100*rebirth)
        bavis_time += 1
        if hardcore == 1:
            lives = 1
...
# this is the save feature, which saves the game everytime "bavis" is run
        ...
        filesave.write('Bavis amount: '+str(bavis_counter)+' Golden Bavis amount: '+str(g_bavis_counter)+' Multiplier: '+str(multiplier)+' Lives: '+str(lives)+' Hardcore: '+str(hardcore)+' Luck: '+str(luck)+' Prestige: '+str(prestige)+' Rebirths: '+str(rebirth)+' Bavis amounts: '+str(bavis_time))
        filesave.close()
...
# this is the randomizer for the golden bavis.
        ...
        if g_bavis_decide == 1:
            ...
            g_bavis_counter += 1 + (0.10*prestige) + (10*rebirth)
...
# these commands are "get" commands. these commands allow you to get the value of something.
    ...
    # this is the load command, to load a previous save.
        ...
        line1 = line.split()
        bavis_counter = int(line1[2])
        g_bavis_counter = int(line1[6])
        multiplier = int(line1[8])
        lives = int(line1[10])
        hardcore = int(line1[12])
        luck = int(line1[14])
        prestige = int(line1[16])
        rebirth = int(line1[18])
        bavis_time = int(line1[20])
