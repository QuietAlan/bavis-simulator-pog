...
# the bavis counter.
# this counts everytime you say bavis.
bavis_counter = 0
g_bavis_counter = 0
multiplier = 1
lives = 3
...
# then if the user replies with "bavis", one "bavis" gets added to the bavis_counter variable.
    if bavis == 'bavis':
        bavis_counter += 1*multiplier
        if hardcore == 1:
            lives = 1
# this is the save feature, which saves the game everytime "bavis" is run
        filesave = open("Logs.txt", "w")
        filesave.write('Bavis amount: '+str(bavis_counter)+' Golden Bavis amount: '+str(g_bavis_counter)+' Multiplier: '+str(multiplier)+' Lives: '+str(lives)+' Hardcore: '+str(hardcore)+' Luck: '+str(luck))
        filesave.close()
...
# this is the shop results, when you buy something.
        ...
        if shopbuy == '1' and bavis_counter >= 50:
            ...
            # this is a cutscene
        elif shopbuy == '10' and g_bavis_counter >= 1000:
            g_bavis_counter -= 1000
